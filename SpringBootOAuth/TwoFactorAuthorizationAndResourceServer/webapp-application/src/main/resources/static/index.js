...
function getToken(username, password) {
	$.ajax({
		  type: "POST",
		  url: "http://localhost:6000/oauth/token",
		  data: "username="+username+"&password="+password+"&grant_type=password",
		  beforeSend: function(request) {
			    request.setRequestHeader("Authorization", "Basic "+btoa("hareram:hareram@singh"));
			    request.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			  },
		  success: function(msg){   
		        if(JSON.parse(JSON.stringify(msg)).is_2fa_enabled=="Y") {
		        	
		        	var email_two_fa_url = "http://localhost:7000/users/"+JSON.parse(JSON.stringify(msg)).id
	        		+"/emails/"+JSON.parse(JSON.stringify(msg)).email_id+"/tfa";
		        	
		        	var mobile_two_fa_url = "http://localhost:7000/users/"+JSON.parse(JSON.stringify(msg)).id
	        		+"/mobilenumbers/"+JSON.parse(JSON.stringify(msg)).mobile+"/tfa";
		        	
		        	var verify_2fa_url = "http://localhost:7000/users/"+JSON.parse(JSON.stringify(msg)).id
	        		+"/codes/";
